...
//        for (String b : result) {
//            finalresult.add(Integer.parseInt(b));
//        }
//        return finalresult;
//    }
...
//        }
//        String current = "";
//        int minimumcharacter = s.length();
//        for(int  i = index;i<s.length();i++){
//            current+=s.charAt(i);
//            int currentextra  = set.contains(current)?0:current.length();
//            int newExtra = recursion(index+1,s,set);
//            int total = currentextra+newExtra;
//            minimumcharacter = Math.min(minimumcharacter,total);
//        }
...
//    }
    ...
    private int recursion(int index, String s, Set<String> set) {
        ...
        int result = 1 + recursion(index + 1, s, set);
        for (int i = index; i < s.length(); i++) {
            String sub = s.substring(index, i - index + 1);
            if (set.contains(sub)) {
                result = Math.min(result, recursion(i + 1, s, set));
            }
        }
        return result;
    }

    public static void dfs(int node, boolean[] visited, HashMap<Integer, ArrayList<Integer>> adj) {
        visited[node] = true;
    ...
    public static void bfs(int node, boolean[] visited, HashMap<Integer, ArrayList<Integer>> adj) {
    ...
    public static void main(String[] args) {
        ...
        // Add edges according to the given graph structure
        adj.get(1).add(2);
        adj.get(1).add(3);
